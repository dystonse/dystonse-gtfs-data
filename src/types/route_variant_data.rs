...
impl TreeData for RouteVariantData {
    fn save_tree(&self, dir_name: &str, format: &SerdeFormat, file_levels: usize) -> FnResult<()> {
        if file_levels == 0 {
            ...
            self.general_delay_arrival.save_to_file(dir_name, "general_delay_arrival", format)?;
            self.general_delay_departure.save_to_file(dir_name, "general_delay_departure", format)?;
