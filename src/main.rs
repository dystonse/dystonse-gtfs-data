...
fn main() -> FnResult<()> {
    ...
    let mut gtfs_schedule_filename = gtfs_schedule_filename_oss
        .to_str()
        .expect("invalid OsString");
    ...
    if gtfs_schedule_filename == "-v" {
        ...
        gtfs_schedule_filename = gtfs_schedule_filename_oss
            .to_str()
            .expect("invalid OsString");
    }
    ...
    let gtfs_realtime_filenames: Vec<String> = args
        .map(|arg| String::from(arg.to_str().expect("invalid OsString")))
        .collect();
